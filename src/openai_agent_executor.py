...
class OpenAIAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        while iteration < max_iterations:
            ...
            # 🔥 FALLBACK: directly call your tool
            try:
                if "analyze_dataset" in self.tools:
                    method = self.tools["analyze_dataset"]
                    result = method(message_text)
                    if inspect.iscoroutine(result):
                        result = await result

                    fallback_text = f"""
        ⚠️ OpenAI API unavailable (quota issue)
        ...
        """
                    ...
                    fallback_text = "Fallback failed: tool not found"

            except Exception as fallback_error:
                fallback_text = f"Fallback also failed: {fallback_error}"

            error_parts = [TextPart(text=fallback_text)]
    ...
    async def execute(
        ...
        logger.debug('[dataset-lie-detector] execute exiting')
