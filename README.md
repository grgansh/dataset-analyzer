...
- `dataset-lie-detector` - Your agent's name (e.g., "a2a-weather-agent")
- `Detects bias, imbalance, and misleading patterns in datasets` - Brief description of your agent
- `dataset-lie-detector` - Docker container name
- `{{AGENT_PORT}}` - Default port (e.g., 5000)
- `{{AGENT_DEFAULT_PORT}}` - Default port as integer (e.g., 5000)

#### Agent Skills & Capabilities
- `dataset_audit` - Unique skill identifier (e.g., 'weather_agent')
- `Dataset Audit` - Human-readable skill name (e.g., 'Weather Agent')
- `Analyzes dataset reliability` - What your agent does
- `tags=["data", "analysis", "bias", "ml"],` - List of tags (e.g., ['weather', 'forecast', 'temperature'])
- `[
...
- `dataset_toolset` - Your toolset module name (e.g., "weather_toolset")
- `DatasetLieDetector` - Your toolset class name (e.g., "WeatherToolset")
- `"Analyzes datasets to detect bias, missing values, duplicates, imbalance, and potential risks before machine learning"` - Brief description of your toolset
...
1. **Find and Replace Placeholders:**
   ...
   find . -name "*.py" -o -name "*.toml" -o -name "*.yml" -o -name "*.md" | xargs sed -i 's/dataset-lie-detector/my-cool-agent/g'
   ```
